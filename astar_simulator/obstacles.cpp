...
bool Obstacles::isSegmentInObstacle(Vector2f &p1, Vector2f &p2)
{
    ...
    for(int i = 0; i < (int)obstacles.size(); i++) {
        ...
        QLineF lseg1(obstacles[i].first.x(), obstacles[i].first.y(), obstacles[i].first.x() + length + 2*BOT_RADIUS, obstacles[i].first.y());
        QLineF lseg2(obstacles[i].first.x(), obstacles[i].first.y(), obstacles[i].first.x(), obstacles[i].first.y() + breadth + 2*BOT_RADIUS);
        QLineF lseg3(obstacles[i].second.x(), obstacles[i].second.y(), obstacles[i].second.x(), obstacles[i].second.y() - breadth - 2*BOT_RADIUS);
        QLineF lseg4(obstacles[i].second.x(), obstacles[i].second.y(), obstacles[i].second.x() - length - 2*BOT_RADIUS, obstacles[i].second.y());
